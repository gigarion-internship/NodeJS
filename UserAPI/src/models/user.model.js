...
module.exports = {
    ...
    getManyByTitle: async username => await prisma[target].findMany({ where: { username: { contains: username } } }),
    // Hàm để lấy một ghi chú duy nhất dựa trên tiêu đề
    getByTitle: async username => await prisma[target].findUnique({ where: { username } }),
